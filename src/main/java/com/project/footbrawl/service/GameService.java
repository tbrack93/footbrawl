...
public class GameService {
	...
	private static List<Integer> diceRolls = new ArrayList<>(
			Arrays.asList(new Integer[] { 6, 6, 1, 6, 6, 6, 6, 1, 1, 6, 6, 4, 1, 6, 6, 6, 6, 6, 6, 6, 6, 6 }));
...
//		}
	...
	public void catchBallAction(PlayerInGame player, boolean accuratePass) {
		...
		if (roll >= needed) {
			...
			runnableLocation = new int[][] { player.getLocation(), {} };
		...
		if (roll == 1 || roll + modifier <= 1) { // on a natural 1 or 1 after modifiers
			...
			if (interceptor != null) {
				...
				Runnable task2 = new Runnable() {
					@Override
					public void run() {
						sender.sendRollResult(game.getId(), thrower.getId(), thrower.getName(), "THROW", needed, rolled, "intercepted",
								thrower.getLocation(), target.getLocation(), null, thrower.getTeam(), "N", reroll);
					}
				};
				taskQueue.add(task2);
	...
	public void carryOutReroll(int playerId, int team, String rerollChoice) {
		...
		if (!rerollChoice.contains("Don't reroll")) {
			...
			awaitingReroll = null;
			taskQueue.pop().run();
			if (rollType == "INTERCEPT") {
				return;
			}
			if (rollType == "BLOCK" || rollType == "THROW") {
